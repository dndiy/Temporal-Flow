  ...
  {#if show}
    <div class="fixed inset-0 bg-black/50 dark:bg-black/60 z-50 flex items-center justify-center">
      <div class="bg-white dark:bg-neutral-800 rounded-lg max-w-2xl w-full overflow-auto shadow-lg">
        <div class="p-5">
          <h3 class="text-xl font-semibold text-black/80 dark:text-white/80 mb-2">Export Community Configuration</h3>
          <p class="text-neutral-600 dark:text-neutral-400 mb-4">
            Download your community configuration to save it to your project. This file should be placed in your project's config directory.
          </p>
          ...
          <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 rounded-md p-3 text-amber-800 dark:text-amber-200 text-sm mb-6">
            <div class="flex">
              ...
              <div>
                ...
                <p class="mt-1">Download this file and place it in your project's <code>src/config/</code> directory as <code>community.config.ts</code>. This enables the community page to use your configured settings.</p>
              </div>
            </div>
          </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <ul class="text-neutral-500 dark:text-neutral-400 mt-1 list-disc pl-4">
                    {#if communityConfig.discord.enabled}
                      <li>Discord</li>
                    {/if}
                    {#if communityConfig.contact.enabled}
                      <li>Contact Form</li>
                    {/if}
                    {#if communityConfig.newsletter.enabled}
                      <li>Newsletter</li>
                    {/if}
                    {#if communityConfig.events.enabled}
                      <li>Events</li>
                    {/if}
                    {#if communityConfig.guidelines.enabled}
                      <li>Guidelines</li>
                    {/if}
                  </ul>
                </div>
                <div>
                  <p class="font-medium text-neutral-700 dark:text-neutral-300">Hero Options:</p>
                  <ul class="text-neutral-500 dark:text-neutral-400 mt-1 list-disc pl-4">
                    {#each communityConfig.hero.options.filter(option => option.enabled) as option}
                      <li>{option.title}</li>
                    {/each}
                  </ul>
                </div>
